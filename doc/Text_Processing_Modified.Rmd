...
urlfile_hm<-'https://raw.githubusercontent.com/rit-public/HappyDB/master/happydb/data/cleaned_hm.csv'
hm_data <- read_csv(urlfile_hm)

urlfile_dem<-'https://raw.githubusercontent.com/rit-public/HappyDB/master/happydb/data/demographic.csv'
dem_data <- read_csv(urlfile_dem)
```
...
### Step 10 - Adding demographic information of the workers

```{r augmented hm_data, warning=FALSE, message=FALSE}
hm_data <- hm_data %>%
  inner_join(dem_data)
```

### Step 11 - Revise the `age` column

```{r revised age column, warning=FALSE, message=FALSE}
hm_data <- hm_data %>% 
  mutate(age = as.integer(age)) %>%
  filter(age>17, age<=100, !is.na(age))
```

### Step 12 - Remove some useless columns and rows

```{r clean columns and rows, warning=FALSE, message=FALSE}
hm_data <- hm_data %>%
  select(-ground_truth_category, -modified) %>%
  drop_na(age, gender, marital, parenthood) %>%
  filter(!nchar(text)<3) %>%
  filter(text != "tv tv")
```

### Step 13 - Exporting the processed text data into a CSV file

```{r export data}
write_csv(hm_data, "../output/processed_moments_modified.csv")
```
